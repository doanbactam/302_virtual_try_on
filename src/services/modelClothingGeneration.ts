...
export const modelClothingGeneration = async (params: IParams) => {
  ...
  const messages: Array<ChatCompletionMessageParam> = [
    {
      role: "system",
      content: type === "model" ? modelSystemPrompt : clothingSystemPrompt,
    },
    {
      role: "user",
      content:
        params?.age && params?.gender
          ? `${params.gender},${params.age},${describe}`
          : describe,
    },
  ];
...
export const translateIntoEnglish = async (params: {
  apiKey: string;
  text: string;
  modelName: string;
}) => {
...
const onGeneration = async (props: {
  prompt: string;
  proportion: string;
  apiKey: string;
  type: "model" | "clothing";
  ...
  try {
    const body = JSON.stringify({
      ...
      negative_prompt:
        type === "clothing"
          ? `${clothingNegativePrompt}, body, limbs, nsfw ,head`
          : "",
      loras:
        type === "clothing"
          ? []
