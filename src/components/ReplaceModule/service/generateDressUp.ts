...
export const generateDressUp = async (params: Params) => {
  ...
  try {
    ...
    if (params.dressModel === "Kling") {
      ...
      const virtualTryOnResult = await onVirtualTryOn(
        body,
        apiKey,
        params.signal
      );
...
// Virtual-Try-On
const onVirtualTryOn = async (
  body: VirtualTryOn,
  apiKey: string,
  signal?: AbortSignal
) => {
  const result: VirtualTryOnTask = await ky(
    `${env.NEXT_PUBLIC_API_URL}/klingai/kolors-virtual-try-on`,
    {
      method: "post",
      headers: {
        Authorization: `Bearer ${apiKey}`,
...
// Virtual-Try-On（Result query）
export const onVirtualTryOnTask = async (
  id: string,
  apiKey: string,
  signal?: AbortSignal
) => {
  const poll = async () => {
    try {
      ...
      const result: VirtualTryOnTask = await ky
        .get(`${env.NEXT_PUBLIC_API_URL}/klingai/task/${id}/fetch`, {
...
const onVirtualTryon2 = async (body: VirtualTryon, apiKey: string) => {
  const result: VirtualTryOnTask = await ky(
    `${env.NEXT_PUBLIC_API_URL}/302/submit/virtual-tryon`,
    {
      method: "post",
      headers: {
        Authorization: `Bearer ${apiKey}`,
...
// Virtual-Tryon（Result query）
export const onVirtualTryonTask2 = async (
  id: string,
  apiKey: string,
  signal?: AbortSignal
) => {
  const poll = async () => {
    ...
    try {
      const result: any = await ky
        .get(`${env.NEXT_PUBLIC_API_URL}/302/submit/virtual-tryon`, {
          headers: { Authorization: `Bearer ${apiKey}` },
          searchParams: {
            request_id: id,
          },
      ...
      if (
        ...
        result?.detail === "Request is still in progress"
      ) {
