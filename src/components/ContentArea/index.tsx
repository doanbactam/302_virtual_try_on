...
import { onVirtualTryOnTask, onVirtualTryonTask2 } from "../ReplaceModule/service/generateDressUp";
import { deleteFinishedProductData, getFinishedProductLsit, IFinishedProduct, updateFinishedProductData } from "@/api/indexDB/FinishedProductDB";
...
export const ContentArea = () => {
    ...
    const [{ finishedProduct, exhibitData, activeIndex, performTasks }, setConfig] = useAtom(userAtom);
    ...
    const onDelete = async (id?: number) => {
        if (!id) return;
        await deleteFinishedProductData(id).then(() => {
            ...
            if (finishedProduct.length > 1) {
                const findSucceededItem = (startIndex: number, direction: 1 | -1) => {
                    for (let i = startIndex; i >= 0 && i < finishedProduct.length; i += direction) {
                        if (finishedProduct[i].id !== id && finishedProduct[i].status === 'succeed') {
            ...
            setConfig((v) => ({ ...v, exhibitData: data, finishedProduct: newData }))
        }).catch(() => {
            toast.error(t('deleteError'));
        })
    }

    const startPolling = useCallback(async (item: IFinishedProduct) => {
                    ...
                    try {
                        if (result?.status === 'succeeded') {
                            if (result?.output) {
                                ...
                                await updateFinishedProductData(item.id!, { url, status: 'succeed' });
                            } else {
                                await updateFinishedProductData(item.id!, { ...item, status: 'failed', error: false });
                            }
                        }
        ...
        if (item.dressModel === 'Virtual-Tryon') {
            onVirtualTryonTask2(item.taskId, apiKey || '', abortController.signal).then(async res => {
                if (res?.image?.url) {
    ...
    return (
        <div className="flex flex-col lg:h-[calc(100vh-100px)] h-full">
            <div className="flex-1 flex flex-col items-center justify-center gap-5 overflow-hidden relative">
                {
                    exhibitData?.url &&
                    <div className=" absolute top-2 w-full flex items-center justify-between px-2">
                        ...
                        <div className="flex items-center gap-2">
                            <Button variant="icon" size="icon" style={{ boxShadow: '0 0 2px 0px #00000054' }} onClick={() => handleDownload(exhibitData?.url || '')}><HiDownload /></Button>
                            <Button variant="icon" size="icon" style={{ boxShadow: '0 0 2px 0px #00000054' }} onClick={() => setVisible(true)} ><BsArrowsFullscreen /></Button>
                            <Button variant="icon" size="icon" style={{ boxShadow: '0 0 2px 0px #00000054' }} onClick={() => onDelete(exhibitData.id)}><FaRegTrashAlt className="text-red-600" /></Button>
                        </div>
                    </div>
                }
                <img className={`object-contain ${exhibitData?.url && 'h-full'}`} src={exhibitData?.url || "/images/global/empty.png"} />
                {!exhibitData?.url && <p className="text-2xl">{t('empty')}</p>}
            </div>
            <div ref={scrollContainerRef} className="h-[150px] min-h-[150px] overflow-x-auto overflow-y-hidden border-t p-3 flex items-center gap-5">
                {
                        ...
                        <div
                            ...
                            onClick={() => {
                                if (item.status === 'succeed') {
                                    setConfig((v) => ({ ...v, exhibitData: item, activeIndex: index }))
                                }
                            }}
                            className={`h-[130px] w-[130px] min-w-[130px] border relative group cursor-pointer flex justify-center ${exhibitData?.url === item.url && 'border-[#8e47f0]'}`}
                        >
                            <img src={item.url} className="h-full w-auto object-contain" />
                            <IoIosCloseCircleOutline
                                onClick={() => onDelete(item.id)}
                                className="absolute right-0 top-0 text-red-600 text-[20px] cursor-pointer hidden group-hover:!block z-10"
                            />
                            {
                                item.status === 'submitted' &&
                                <div className='bg-[hsl(var(--background-backdrop))] w-full backdrop-blur-sm rounded-xl flex justify-center items-center'>
                                    {
                                        item?.error ?
                                            <div className="w-full h-full  flex justify-center items-center" onClick={(e) => {
                                                e.preventDefault();
                                                e.stopPropagation();
                                                startPolling(item);
                                            }}>
                            ...
                            {
                                item.status === 'failed' &&
                                <div className="text-red-600 flex flex-col justify-center items-center text-sm gap-2">
